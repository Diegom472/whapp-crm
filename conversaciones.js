...
function iniciarAudio() {
  navigator.mediaDevices.getUserMedia({ audio: true }).then(stream => {
    ...
    // Detectar el formato soportado. WhatsApp acepta OGG/Opus para notas de voz.
    // Priorizamos ogg para que Meta lo acepte al reenviar.
    const formatos = [
...
async function enviarAudioPausado() {
  ...
  if (r2url) {
    ...
    // Si Meta rechazó el audio, avisar con el motivo
    if (resp && resp.error) {
